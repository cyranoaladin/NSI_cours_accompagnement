...
class ARIAService:
    ...
    def detect_message_type(self, message: str) -> str:
        ...
        if any(word in message_lower for word in ['bonjour', 'salut', 'hello', 'bonsoir']):
            return 'greeting'
        elif any(word in message_lower for word in ['mathématiques', 'maths', 'calcul', 'équation', 'fonction']):
            return 'math'
        elif any(word in message_lower for word in ['améliorer', 'notes', 'résultats', 'conseils']):
            return 'study_tips'
        elif any(word in message_lower for word in ['bac', 'baccalauréat', 'examen', 'préparer']):
            return 'bac_prep'
        elif any(word in message_lower for word in ['matières', 'matière', 'renforcer', 'faible']):
            return 'subjects'
        else:
            return 'default'
    ...
    def generate_chat_response(self, student_profile: Dict, message: str, context: Dict = None) -> Dict[str, Any]:
        ...
        try:
            ...
            badges = [
                {"text": f"🎯 Style: {learning_style.title()}", "class": "bg-blue-100 text-blue-800"},
                {"text": f"📊 Confiance: {random.randint(85, 98)}%", "class": "bg-green-100 text-green-800"}
            ]

            # Ajout de badges spécifiques selon le type de message
            if message_type == 'math':
                badges.append({"text": "🔢 Spécialiste Maths", "class": "bg-purple-100 text-purple-800"})
            elif message_type == 'bac_prep':
                badges.append({"text": "🎓 Coach Bac", "class": "bg-orange-100 text-orange-800"})
            ...
            return {
                ...
                "badges": [{"text": "⚠️ Mode dégradé", "class": "bg-orange-100 text-orange-800"}],
