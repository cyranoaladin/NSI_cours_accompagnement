...
class ARIAService:
    ...
    def __init__(self):
        ...
        self.subjects = {
            'mathematiques': ['algèbre', 'géométrie', 'analyse', 'probabilités',
                              'statistiques'],
            'physique': ['mécanique', 'thermodynamique', 'électricité', 'optique',
                         'physique_moderne'],
            'chimie': ['chimie_organique', 'chimie_inorganique', 'thermochimie',
                       'cinétique'],
            'francais': ['littérature', 'grammaire', 'expression_écrite',
                         'analyse_texte'],
            'nsi': ['algorithmique', 'programmation', 'bases_données', 'réseaux',
                    'architecture'],
            'philosophie': ['métaphysique', 'épistémologie', 'éthique', 'politique',
                            'esthétique']
        }
        ...
        try:
            if self.api_key and self.api_key.startswith('sk-'):
                ...
                if os.environ.get('FLASK_ENV') != 'testing':
                    logger.warning(
                        "Clé OpenAI non configurée - Mode simulation activé"
                    )
                self.has_openai = False
        except ImportError:
    ...
    def detect_message_type(self, message: str) -> str:
        ...
        if any(word in message_lower
            ...
            return 'greeting'
        if any(word in message_lower
               for word in ['mathématiques', 'maths', 'calcul',
                            'équation', 'fonction']):
            return 'math'
        if any(word in message_lower for word in ['améliorer', 'notes', 'résultats', 'conseils']):
            return 'study_tips'
        if any(word in message_lower for word in ['bac', 'baccalauréat', 'examen', 'préparer']):
            return 'bac_prep'
        return 'default'
    ...
    def _build_system_prompt(self, student_profile: Dict) -> str:
        ...
        return ("Tu es ARIA, l'assistant IA personnel de Nexus Réussite, "
    ...
    def _generate_badges(self, student_profile: Dict, message: str) -> List[Dict]:
        ...
        if message_type == 'math':
            badges.append({"text": "🔢 Spécialiste Maths",
                          "class": "bg-purple-100 text-purple-800"})
        elif message_type == 'bac_prep':
            badges.append({"text": "🎓 Coach Bac",
    ...
    def _generate_openai_content(self, student_profile: Dict, subject: str,
        ...
        try:
            prompt = (f"Génère du contenu éducatif personnalisé pour:\n"
    ...
    def analyze_learning_style(self, responses: Dict) -> Dict[str, Any]:
        ...
        for _question, answer in responses.items():  # pylint: disable=unused-variable
            if isinstance(answer, str):
                ...
                if any(word in answer_lower for word in ['lire', 'écrire', 'texte', 'noter']):
    ...
    def _generate_performance_recommendations(self, score: float,
        ...
        if time_spent < 300:  # Moins de 5 minutes
            ...
            recommendations.append(
                "Travaillez l'efficacité et la gestion du temps."
            )
    ...
    def evaluate_progress(self, student_id: int,  # pylint: disable=unused-argument
        ...
        if trend == "amélioration":
            ...
            if average_score >= 85:
                recommendations.append(
                    "Performance stable et élevée. Essayez des défis plus "
                    "complexes."
                )
        ...
        return {
            ...
            "assessment": {
                "current_level": level,
                "strengths": (["Régularité dans le travail"]
                              if trend != "déclin" else []),
                "areas_for_improvement": (["Consistency"] if trend == "déclin"
