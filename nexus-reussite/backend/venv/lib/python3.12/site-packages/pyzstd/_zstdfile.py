...
from pyzstd import ZstdCompressor, _ZSTD_DStreamSizes, _ZstdFileReader, _ZstdFileWriter
...
class ZstdFile(io.BufferedIOBase):
    ...
    def read(self, size=-1):
        ...
        If size is negative or omitted, read until EOF is reached. Returns b"" if the
        file is already at EOF.
        """
        if size is None:
            size = -1
        try:
    ...
    def read1(self, size=-1):
        ...
        reads from the underlying stream. Reads up to a buffer's worth of data if size
        is negative.
    ...
    def readinto1(self, b):
        """Read bytes into b, while trying to avoid making multiple reads from the
        underlying stream.
    ...
    def readline(self, size=-1):
        ...
        The terminating newline (if present) is retained. If size is non-negative, no
        more than size bytes will be read (in which case the line may be incomplete).
        Returns b'' if already at EOF.
        """
        if size is None:
            size = -1
        try:
    ...
    def peek(self, size=-1):
        ...
        Always returns at least one byte of data, unless at EOF. The exact number of
        bytes returned is unspecified.
        """
