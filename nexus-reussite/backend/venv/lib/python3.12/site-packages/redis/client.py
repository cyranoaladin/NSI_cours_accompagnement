...
class Redis(RedisModuleCommands, CoreCommands, SentinelCommands):
    ...
    Pipelines derive from this, implementing how the commands are sent and received to
    the Redis server. Based on configuration, an instance will either use a
    ConnectionPool, or Connection object to talk to redis.
    ...
    def from_pool(
        ...
        The Redis client will take ownership of the connection pool and close it when
    ...
    def pipeline(self, transaction=True, shard_hint=None) -> "Pipeline":
        """Return a new pipeline object that can queue multiple commands for later
        execution.

        ``transaction`` indicates whether all commands
    ...
    def transaction(
        ...
        """Convenience method for executing the callable `func` as a transaction while
        watching all keys specified in `watches`.

        The 'func' callable should expect a single argument which is a Pipeline object.
        """
    ...
    def lock(
        ...
        """Return a new Lock object using key ``name`` that mimics the behavior of
        threading.Lock.
    ...
    def pubsub(self, **kwargs):
        ...
        With this object, you can subscribe to channels and listen for messages that get
        published to them.
        """
        return PubSub(
    ...
    def _close_connection(self, conn) -> None:
        ...
        The supported exceptions are already checked in the retry object so we don't
        need to do it here.

        After we disconnect the connection, it will try to reconnect and do a health
        check as part of the send_command logic(on connection level).
        """
        conn.disconnect()
...
class Monitor:
    ...
    next_command() method returns one command from monitor listen() method yields
...
class PubSub:
    ...
    After subscribing to one or more channels, the listen() method will block until a
    message arrives on one of the subscribed channels. That message will be returned and
    ...
    def _execute(self, conn, command, *args, **kwargs):
        """Connect manually upon disconnection.
    ...
    def is_health_check_response(self, response) -> bool:
        ...
        If there are no subscriptions redis responds to PING command with a bulk
        response, instead of a multi-bulk with "pong" and the response.
        """
        return response in [
    ...
    def _normalize_keys(self, data) -> Dict:
        """Normalize channel/pattern names to be either bytes or strings based on
        whether responses are automatically decoded.

        this saves us from coercing the value for each message coming in.
        """
    ...
    def psubscribe(self, *args, **kwargs):
        ...
        Patterns supplied as keyword arguments
    ...
    def punsubscribe(self, *args):
        ...
        If empty, unsubscribe from all patterns.
        """
        if args:
            args = list_or_args(args[0], args[1:])
    ...
    def sunsubscribe(self, *args, target_node=None):
        ...
        If empty, unsubscribe from all shard_channels
        """
        if args:
            args = list_or_args(args[0], args[1:])
    ...
    def handle_message(self, response, ignore_subscribe_messages=False):
        ...
        If the channel or pattern was subscribed to with a message handler, the handler
        is invoked instead of a parsed message being returned.
        """
        if response is None:
            return None
        if isinstance(response, bytes):
...
class Pipeline(Redis):
    """Pipelines provide a way to transmit multiple commands to the Redis server in one
    transmission.  This is convenient for batch processing, such as saving all the
    values in a list to Redis.

    All commands executed within a pipeline(when running in transactional mode, which is
    the default behavior) are wrapped with MULTI and EXEC calls. This guarantees all
    commands executed in the pipeline will be executed atomically.

    Any command raising an exception does *not* halt the execution of subsequent
    commands in the pipeline. Instead, the exception is caught and its instance is
    placed into the response list returned by execute(). Code iterating over the
    response list should be able to deal with an instance of an exception as a potential
    value. In general, these will be ResponseError exceptions, such as those raised when
    issuing a command on a key of a different datatype.
    """
    ...
    def multi(self) -> None:
        ...
        End the transactional block with `execute`.
        """
    ...
    def _disconnect_reset_raise_on_watching(
        ...
        """Close the connection reset watching state and raise an exception if we were
        watching.
    ...
    def immediate_execute_command(self, *args, **options):
        """Execute a command immediately, but don't auto-retry on the supported errors
        for retry if we're already WATCHing a variable.

        Used when issuing WATCH or subsequent commands retrieving their values but
        before MULTI is called.
        """
        command_name = args[0]
        conn = self.connection
