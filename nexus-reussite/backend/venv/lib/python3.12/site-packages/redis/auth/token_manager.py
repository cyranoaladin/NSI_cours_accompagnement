...
class TokenManagerConfig:
    ...
    def get_expiration_refresh_ratio(self) -> float:
        """Represents the ratio of a token's lifetime at which a refresh should be
        triggered. For example, a value of 0.75 means the token should be refreshed #
        noqa: E501 when 75% of its lifetime has elapsed (or when 25% of its lifetime
        remains).
    ...
    def get_lower_refresh_bound_millis(self) -> int:
        """Represents the minimum time in milliseconds before token expiration to
        trigger a refresh, in milliseconds. This value sets a fixed lower bound for when
        a token refresh should occur, regardless of the token's total lifetime. If set
        to 0 there will be no lower bound and the refresh will be triggered based on the
        expirationRefreshRatio only.
    ...
    def get_token_request_execution_timeout_in_ms(self) -> int:
        """Represents the maximum time in milliseconds to wait for a token request to
        complete.
...
def _start_event_loop_in_thread(event_loop: asyncio.AbstractEventLoop):
    """Starts event loop in a thread. using. loop.call_later. Used to be able to
    schedule tasks.
