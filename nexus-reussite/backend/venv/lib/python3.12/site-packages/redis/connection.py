...
class AbstractConnection(ConnectionInterface):
    ...
    def register_connect_callback(self, callback):
        ...
        initially or reconnected.

        This allows listeners to issue commands that are ephemeral to the connection,
        for example pub/sub subscription or key tracking.  The callback must be a
        _method_ and will be kept as a weak reference.
        """
    ...
    def deregister_connect_callback(self, callback):
        """De-register a previously registered callback.

        It will no-longer receive notifications on connection events.  Calling this is
        not required when the listener goes away, since the callbacks are kept as weak
        methods.
        """
        try:
...
class ConnectionPool:
    """Create a connection pool. pool's ``If max_connections`` object raises is set,
    then this limit is reached. :py:class:`~redis.exceptions.ConnectionError` when the.
