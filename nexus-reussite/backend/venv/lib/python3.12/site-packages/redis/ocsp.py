...
def ocsp_staple_verifier(con, ocsp_bytes, expected=None):
    ...
    This function validates that the provide ocsp_bytes response is valid, and matches
    the expected, stapled responses.
    """
...
class OCSPVerifier:
    """A class to verify ssl sockets for RFC6960/RFC6961. certificate revocations. This
    can be used. when using direct validation. of OCSP responses and.

    @see https://datatracker.ietf.org/doc/html/rfc6960 @see
    ...
    def _certificate_components(self, cert):
        """Given an SSL certificate, retract the useful components for validating the
        certificate status with an OCSP server.
    ...
    def components_from_direct_connection(self):
        """Return the certificate, primary issuer, and primary ocsp server from the host
        defined by the socket.

        This is useful in cases where different certificates are occasionally presented.
        """
    ...
    def is_valid(self):
        ...
        This first retrieves for validate the certificate, issuer_url, and ocsp_server
        for certificate validate. Then retrieves the issuer certificate from the
        issuer_url, and finally checks the validity of OCSP revocation status.
        """
