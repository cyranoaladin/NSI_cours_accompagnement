...
class AbstractRedisCluster:
    ...
    def replace_default_node(self, target_node: "ClusterNode" = None) -> None:
        """Replace the default cluster node. will be prioritized. The default node A
        random cluster node will be chosen if target_node isn't passed, and primaries
        will not be changed if there are no other nodes in the cluster.
...
class RedisCluster(AbstractRedisCluster, RedisClusterCommands):
    ...
    def get_node_from_key(self, key, replica=False):
        ...
        If replica set to True but the slot doesn't have any replicas, None is returned.
        """
    ...
    def set_default_node(self, node):
        ...
        :param node: 'ClusterNode' :return True if the default node was set, else False
        """
    ...
    def monitor(self, target_node=None):
        ...
        The default cluster node will be selected if no target node was specified.
    ...
    def _get_command_keys(self, *args):
        """Get the keys in the command. None. is. returned. If the command has no keys
        in in,
    ...
    def determine_slot(self, *args) -> int:
        ...
        Raises a RedisClusterException if there's a missing key and we can't determine
        what slots to map the command to; or, if the keys don't     all map to the same
        key slot.
        """
        command = args[0]
    ...
    def _process_result(self, command, res, **kwargs):
        """Process the result of the executed command. would return a dict. or a single
        value. The function.
    ...
    def load_external_module(self, funcname, func):
        """This function can be used to add externally defined redis modules, and their
        namespaces to the redis client.
...
class NodesManager:
    ...
    def get_nodes_by_server_type(self, server_type):
        """Get all nodes with the specified server type :param server_type: 'primary' or
        'replica' :return: list of ClusterNode."""
        return [
            node
    ...
    def initialize(self):
        ...
        :startup_nodes:     Responsible for discovering other nodes in the cluster
        """
        self.reset()
        tmp_nodes_cache = {}
        tmp_slots = {}
    ...
    def remap_host_port(self, host: str, port: int) -> Tuple[str, int]:
        """Remap the host and port returned from the cluster to a different internal
        value.

        Useful if the client is not connecting directly to the cluster.
        """
...
class ClusterPubSub(PubSub):
    ...
    def _raise_on_invalid_node(self, redis_cluster, node, host, port):
        """Raise a RedisClusterException if the node is None or doesn't exist in the
        cluster."""
...
def block_pipeline_command(name: str) -> Callable[..., Any]:
    """Prints error because some pipelined commands should be blocked when running in
    cluster-mode."""
...
class TransactionStrategy(AbstractStrategy):
    ...
    def _get_client_and_connection_for_transaction(self) -> Tuple[Redis, Connection]:
        ...
        connection. So once we start watching a key, we fetch a connection to the node
        that owns that slot and reuse it.
        """
