...
class AuthenticationWrongNumberOfArgsError(ResponseError):
    """An error to indicate that the wrong number of args were sent to the AUTH
    command."""
...
class ClusterError(RedisError):
    """Cluster errors occurred multiple times, resulting in an exhaustion of the command
    execution TTL."""
...
class ClusterDownError(ClusterError, ResponseError):
    ...
    least a hash slot uncovered (no available node is serving it). This way if the
    cluster is partially down (for example a range of hash slots are no longer covered)
    the entire cluster eventually becomes unavailable. It automatically returns
    available as soon as all the slots are covered again.
    """
...
class AskError(ResponseError):
    """Error indicated ASK error received from cluster. that pertain to this hash slot,
    but only if the key When a slot is set as MIGRATING, the node will accept all
    queries in question exists, otherwise the query is forwarded using a -ASK
...
class ClusterCrossSlotError(ResponseError):
    ...
    A CROSSSLOT error is generated when keys in a request don't hash to the same slot.
    """
...
class MovedError(AskError):
    ...
    A request sent to a node that doesn't serve this key will be replayed with a MOVED
    error that points to the correct node.
    """
...
class SlotNotCoveredError(RedisClusterException):
    """This error only happens in the case where the connection pool will try to fetch
    what node that is covered by a given slot.

    If this error is raised the client should drop the current node layout and attempt
    to reconnect and refresh the node layout again
    """
...
class CrossSlotTransactionError(RedisClusterException):
    """Raised when a transaction or watch is triggered in a pipeline and not all keys or
    all commands belong to the same slot."""
...
class InvalidPipelineStack(RedisClusterException):
    ...
    This is most likely a handling error on the stack.
    """
