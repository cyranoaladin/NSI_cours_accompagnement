...
class SearchCommands:
    ...
    def alter_schema_add(self, fields: List[str]):
        """Alter the existing search index by adding new fields. exist. The. index. must
        already.
    ...
    def dropindex(self, delete_documents: bool = False):
        """Drop the index if it exists. documents. Replaced `drop_index` in RediSearch
        2.0. Default. behavior was changed to not delete the indexed.
    ...
    def explain(
        ...
        `FT.EXPLAIN <https://redis.io/commands/ft.explain>`_.
        """  # noqa
    ...
    def sugadd(self, key, *suggestions, **kwargs):
        """Add suggestion terms to the AutoCompleter engine. kwargs["increment"] is true
        and the terms are already in the a score and string. suggestion has If Each
        server's dictionary, we increment their scores.

        For more information see
        `FT.SUGADD <https://redis.io/commands/ft.sugadd/>`_.
        """  # noqa
    ...
    def suglen(self, key: str) -> int:
        ...
        For more information see
        `FT.SUGLEN <https://redis.io/commands/ft.suglen>`_.
        """  # noqa
        return self.execute_command(SUGLEN_COMMAND, key)

    def sugdel(self, key: str, string: str) -> int:
        """Delete a string from the AutoCompleter index. Returns 1 if the string was.
        found and deleted, 0 otherwise.

        For more information see
        `FT.SUGDEL <https://redis.io/commands/ft.sugdel>`_.
        """  # noqa
    ...
    def synupdate(self, groupid: str, skipinitial: bool = False, *terms: List[str]):
        """Updates a synonym group. were indexed after the update The command is used to
        create or update a synonym group with additional terms. Only documents which
        will be affected.
    ...
    def syndump(self):
        ...
        The command is used to dump the synonyms data structure. Returns a list of
        synonym terms and their synonym group ids.

        For more information see
        `FT.SYNDUMP <https://redis.io/commands/ft.syndump>`_.
        """  # noqa
...
class AsyncSearchCommands(SearchCommands):
    ...
    async def spellcheck(self, query, distance=None, include=None, exclude=None):
        ...
        **query**: search query. **distance***: the maximal Levenshtein distance for
        spelling                suggestions (default: 1, max: 4). **include**: specifies
        an inclusion custom dictionary. **exclude**: specifies an exclusion custom
        dictionary.

        For more information see
        `FT.SPELLCHECK <https://redis.io/commands/ft.spellcheck>`_.
        """  # noqa
        cmd = [SPELLCHECK_CMD, self.index_name, query]
        if distance:
            cmd.extend(["DISTANCE", distance])
