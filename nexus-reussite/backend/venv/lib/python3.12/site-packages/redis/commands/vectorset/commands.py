...
class VectorSetCommands(CommandsProtocol):
    ...
    def vsim(
        ...
        """Compare a vector or element ``input``  with the other vectors in a vector set
        ``key``.
    ...
    def vemb(self, key: KeyT, element: str, raw: Optional[bool] = False) -> Union[
        ...
        """Get the approximated vector of an element ``element`` from vector set
        ``key``.
    ...
    def vlinks(
        ...
        """Returns the neighbors for each level the element ``element`` exists in the
        vector set ``key``.
    ...
    def vsetattr(
        ...
        """Associate or remove JSON attributes ``attributes`` of element ``element`` for
        vector set ``key``.
    ...
    def vgetattr(
        ...
        https://redis.io/commands/vgetattr
        """
