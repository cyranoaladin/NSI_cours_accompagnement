...
class ClusterMultiKeyCommands(ClusterCommandsProtocol):
    ...
    def mget_nonatomic(self, keys: KeysT, *args: KeyT) -> List[Optional[Any]]:
        """Splits the keys into different slots and then calls MGET for the keys of
        every slot. This operation will not be atomic if keys belong to more than one
        slot.
    ...
    def mset_nonatomic(self, mapping: Mapping[AnyKeyT, EncodableT]) -> List[bool]:
        ...
        Splits the keys into different slots and then calls MSET for the keys of every
        slot. This operation will not be atomic if keys belong to more than one slot.
    ...
    def _split_command_across_slots(self, command: str, *keys: KeyT) -> int:
        """Runs the given command once for the keys of each slot.
    ...
    def exists(self, *keys: KeyT) -> ResponseT:
        """Returns the number of ``names`` that exist in the whole cluster. up into
        slots and then an EXISTS command is sent for every slot. The keys are. first
        split.
    ...
    def delete(self, *keys: KeyT) -> ResponseT:
        """Deletes the given keys in the cluster. every slot. The keys are first split
        up into slots. and then an DEL command is sent for.
    ...
    def touch(self, *keys: KeyT) -> ResponseT:
        ...
        The keys are first split up into slots and then an TOUCH command is sent for
        every slot
...
class ClusterManagementCommands(ManagementCommands):
    ...
    The class inherits from Redis's core ManagementCommands class and do the required
    adjustments to work with cluster mode
    """
    ...
    def cluster_addslots(
        ...
        Sends to specified node.         :target_node: 'ClusterNode'             The
        node to execute the command on
    ...
    def cluster_addslotsrange(
        ...
        """Similar to the CLUSTER ADDSLOTS command. list of slots to assign to the node,
        while The difference between the two commands is that ADDSLOTS takes a
        ADDSLOTSRANGE takes a list of slot ranges (specified by start and end slots) to
        assign to the node.
    ...
    def cluster_countkeysinslot(self, slot_id: int) -> ResponseT:
        """Return the number of local keys in the specified hash slot Send to node based
        on specified slot_id.
    ...
    def cluster_count_failure_report(self, node_id: str) -> ResponseT:
        """Return the number of failure reports active for a given node Sends to a
        random node.
    ...
    def cluster_delslots(self, *slots: EncodableT) -> List[bool]:
        """Set hash slots as unbound in the cluster. It determines by it self what node.
        the slot is in and sends it there.
    ...
    def cluster_delslotsrange(self, *slots: EncodableT) -> ResponseT:
        """Similar to the CLUSTER DELSLOTS command. to remove from the node, while
        CLUSTER The difference is that CLUSTER DELSLOTS takes a list of hash slots
        DELSLOTSRANGE takes a list of slot ranges to remove from the node.
    ...
    def cluster_failover(
        ...
        """Forces a slave to perform a manual failover of its master Sends to specified
        node.
    ...
    def cluster_info(self, target_nodes: Optional["TargetNodesT"] = None) -> ResponseT:
        """Provides info about Redis Cluster node state. no target node is specified.
        The command will be sent to a. random node in the cluster if.
    ...
    def cluster_keyslot(self, key: str) -> ResponseT:
        """Returns the hash slot of the specified key Sends to random node in the
        cluster.
    ...
    def cluster_meet(
        ...
        Sends to specified node.         For more information see
    ...
    def cluster_nodes(self) -> ResponseT:
        ...
        Sends to random node in the cluster.         For more information see
    ...
    def cluster_reset(
        ...
        If 'soft' is True then it will send 'SOFT' argument If 'soft' is False then it
        will send 'HARD' argument
    ...
    def cluster_setslot_stable(self, slot_id: int) -> ResponseT:
        """Clears migrating / importing state from the slot. and sends it there. It
        determines by it self. what node the slot is in.
    ...
    def readonly(self, target_nodes: Optional["TargetNodesT"] = None) -> ResponseT:
        """Enables read queries. to the default cluster node if. target_nodes is not
        specified. The command will be sent.
    ...
    def readwrite(self, target_nodes: Optional["TargetNodesT"] = None) -> ResponseT:
        """Disables read queries. sent to the default cluster node. if target_nodes is
        not specified. The command will be.
...
class ClusterDataAccessCommands(DataAccessCommands):
    ...
    The class inherits from Redis's core DataAccessCommand class and do the required
    adjustments to work with cluster mode
    """
