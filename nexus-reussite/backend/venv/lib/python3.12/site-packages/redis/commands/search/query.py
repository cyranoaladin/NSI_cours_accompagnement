...
class Query:
    """Query is used to build complex queries that have more parameters than just the
    query string. The query string is set in the constructor, and other options have
    setter functions.
    ...
    def __init__(self, query_string: str) -> None:
        ...
        The query string is set in the constructor, and other options have setter
        functions.
        """
    ...
    def limit_ids(self, *ids) -> "Query":
        """Limit the results to a specific set of pre-known document ids of any
        length."""
        self._ids = ids
        return self
    ...
    def summarize(
        ...
        """Return an abridged format of the field, containing only the segments of the
        field which contain the matching term(s).
    ...
    def slop(self, slop: int) -> "Query":
        """Allow a maximum of N intervening non matched terms between phrase terms (0
        means exact phrase)."""
        self._slop = slop
        return self
    ...
    def in_order(self) -> "Query":
        """Match only documents where the query terms appear in the same order in the
        document.
    ...
    def scorer(self, scorer: str) -> "Query":
        """Use a different scoring function to evaluate document relevance. Default is.
        `TFIDF`.
    ...
    def add_filter(self, flt: "Filter") -> "Query":
        """Add a numeric or geo filter to the query. **Currently only one of each
        filter. is supported by the engine**
