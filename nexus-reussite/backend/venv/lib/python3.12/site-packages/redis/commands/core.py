...
class ACLCommands(CommandsProtocol):
    ...
    def acl_help(self, **kwargs) -> ResponseT:
        """The ACL HELP command returns helpful text describing the different
        subcommands.
    ...
    def acl_log(self, count: Optional[int] = None, **kwargs) -> ResponseT:
        ...
        :rtype: List. For more information see https://redis.io/commands/acl-log
        """
        args = []
    ...
    def acl_log_reset(self, **kwargs) -> ResponseT:
        ...
        :rtype: Boolean. For more information see https://redis.io/commands/acl-log
        """
        args = [b"RESET"]
        return self.execute_command("ACL LOG", *args, **kwargs)
...
class ManagementCommands(CommandsProtocol):
    ...
    def auth(self, password: str, username: Optional[str] = None, **kwargs):
        ...
        If you do not pass username, Redis will try to authenticate for the "defaul
        user. If you do pass username, it will authenticate for the given user. For more
        information see "
    ...
    def bgsave(self, schedule: bool = True, **kwargs) -> ResponseT:
        """Tell the Redis server to save its data to disk. Unlike save(), this method is
    ...
    def client_kill_filter(
        ...
        **kwargs,
        ...
        """Disconnects client(s) using a variety of filter options :param _id: Kills a
        client by its unique ID field :param _type: Kills a client by type where type is
        one of 'normal', 'master', 'slave' or 'pubsub' :param addr: Kills a client by
        its 'address:port' :param skipme: If True, then the client calling the command
        will not get killed even if it is identified by one of the filter options.

        If skipme is not provided, the server defaults to skipme=True
    ...
    def client_list(
        ...
        """Returns a list of currently connected clients. returned. If type of client
        specified, only that type will be.

        :param _type: optional. one of the client types (normal, master, replica,
            pubsub)
        :param client_id: optional. a list of client ids For more information see
    ...
    def client_getredir(self, **kwargs) -> ResponseT:
        """Returns the ID (an integer) of the client to whom we are redirecting tracking
        notifications.
    ...
    def client_tracking_on(
        ...
        """Turn on the tracking mode. the options look at. client_tracking func. For
    ...
    def client_tracking_off(
        ...
        """Turn off the tracking mode. the options look at. client_tracking func. For
        more information about.
    ...
    def client_tracking(
        ...
        **kwargs,
        ...
        """Enables the tracking feature of the Redis server, that is used for server
    ...
    def client_trackinginfo(self, **kwargs) -> ResponseT:
        """Returns the information about the current client connection's use of the
    ...
    def client_unblock(
        ...
        """Unblocks a connection by its client id. If ``error`` is False (default), the
        client If ``error`` is True, unblocks the client with a special error message.
        is unblocked using the regular timeout mechanism.
    ...
    def client_no_touch(self, mode: str) -> Union[Awaitable[str], str]:
        ...
        LRU/LFU of the keys they access. # When turned on, the current client will not
        change LFU/LRU stats, # unless it sends the TOUCH command.
    ...
    def command_docs(self, *args):
        """This function throws a NotImplementedError since it is intentionally not
        supported."""
        raise NotImplementedError(
    ...
    def lastsave(self, **kwargs) -> ResponseT:
        """Return a Python datetime object representing the last time the Redis database
        was saved to disk.
    ...
    def migrate(
        ...
        **kwargs,
        ...
        """Migrate 1 or more keys from the current Redis server to a different server
        specified by the ``host``, ``port`` and ``destination_db``.
    ...
    def save(self, **kwargs) -> ResponseT:
        """Tell the Redis server to save its data to disk, blocking until the save is
        complete.
    ...
    def slaveof(
        ...
        ``host`` and ``port``. If called without arguments, the instance is promoted to
        a master instead.
    ...
    def slowlog_get(self, num: Optional[int] = None, **kwargs) -> ResponseT:
        """Get the entries from the slowlog. items. If ``num`` is. specified, get the.
        most recent ``num``
    ...
    def wait(self, num_replicas: int, timeout: int, **kwargs) -> ResponseT:
        """Redis synchronous replication That returns the number of replicas that
        processed the query when we finally have at least ``num_replicas``, or when the
        ``timeout`` was reached.
    ...
    def waitaof(
        ...
        """This command blocks the current client until all previous write commands by
        that client are acknowledged as having been fsynced to the AOF of the local
        Redis and/or at least the specified number of replicas.
...
class AsyncManagementCommands(ManagementCommands):
    ...
    async def shutdown(
        ...
        **kwargs,
        ...
        """Shutdown the Redis server. configured, a data flush will be data will be
        flushed before shutdown.  If the "save" option is set, If Redis has persistence
        attempted even if there is no persistence configured.  If the "nosave" option is
        set, no data flush will be attempted.  The "save" and "nosave" options cannot
        both be set.
...
class BitFieldOperation:
    ...
    def overflow(self, overflow: str):
        """Update the overflow algorithm of successive INCRBY operations :param
        overflow: Overflow algorithm, one of WRAP, SAT, FAIL.
    ...
    def execute(self) -> ResponseT:
        """Execute the operation(s) in a single BITFIELD command.

        The return value is a list of values corresponding to each operation. If the
        client used to create this instance was a pipeline, the list of values will be
        present within the pipeline's execute.
        """
...
class BasicKeyCommands(CommandsProtocol):
    ...
    def append(self, key: KeyT, value: EncodableT) -> ResponseT:
        """Appends the string ``value`` to the value at ``key``. of ``value``. If
        ``key`` Returns the new length of the doesn't already exist, create it with a
        value value at ``key``.
    ...
    def bitcount(
        ...
        """Returns the count of set bits in the value of ``key``. which bytes to
        consider Optional. ``start`` and ``end`` parameters indicate.
    ...
    def bitfield(
        ...
        """Return a BitFieldOperation instance to conveniently construct one or more
        bitfield operations on ``key``.
    ...
    def bitfield_ro(
        ...
        """Return an array of the specified bitfield values where the first value is
        found using ``encoding`` and ``offset`` parameters and remaining values are
        result of corresponding encoding/offset pairs in optional list ``items`` Read-
        only variant of the BITFIELD command.
    ...
    def bitop(self, operation: str, dest: KeyT, *keys: KeyT) -> ResponseT:
        """Perform a bitwise operation using ``operation`` between ``keys`` and store
        the result in ``dest``.
    ...
    def bitpos(
        ...
        """Return the position of the first bit set to 1 or 0 in a string. defines
        search range. The range is interpreted as a range of bytes and ``start`` ``end``
        and not a range of bits, so start=0 and end=2 means to look at the first three
        bytes.
    ...
    def dump(self, name: KeyT) -> ResponseT:
        """Return a serialized version of the value stored at the specified key. not
        exist a nil bulk reply is returned. If key. does.
    ...
    def expireat(
        ...
        """Set an expire flag on key ``name`` with given ``option``. indicating unix
        time or a Python ``when`` datetime object. can be represented. as an integer.
    ...
    def getdel(self, name: KeyT) -> ResponseT:
        """Get the value at key ``name`` and delete the key. deletes This command the
        key on success (if and only is similar to GET, except for the fact that it also
    ...
    def getex(
        ...
        """Get the value of key and optionally set its expiration. command with
        additional options. All time parameters can be GETEX is similar to GET, but is a
        write given as datetime.timedelta or integers.
    ...
    def __getitem__(self, name: KeyT):
        """Return the value at key ``name``, raises a KeyError if the key doesn't
        exist."""
        value = self.get(name)
    ...
    def getset(self, name: KeyT, value: EncodableT) -> ResponseT:
        """Sets the value at key ``name`` to ``value`` and returns the old value at key
        ``name`` atomically.

        As per Redis 6.2, GETSET is considered deprecated. Please use SET with GET
        parameter in new code.
    ...
    def incrby(self, name: KeyT, amount: int = 1) -> ResponseT:
        """Increments the value of ``key`` by ``amount``. exists, the value will be
        initialized. as ``amount`` If no key.
    ...
    def incrbyfloat(self, name: KeyT, amount: float = 1.0) -> ResponseT:
        """Increments the value at key ``name`` by floating ``amount``. value will be
        initialized as ``amount`` If no key. exists, the.
    ...
    def lmove(
        ...
        """Atomically returns and removes the first/last element of a list, pushing it
        as the first/last element on the destination list. Returns the element being
        popped and pushed.
    ...
    def mset(self, mapping: Mapping[AnyKeyT, EncodableT]) -> ResponseT:
        """Sets key/values based on a mapping. is a dictionary of can be cast to a
        key/value pairs. Both keys and values string should be strings or types that
        Mapping via str().
    ...
    def msetnx(self, mapping: Mapping[AnyKeyT, EncodableT]) -> ResponseT:
        ...
        Mapping is a dictionary of key/value pairs. Both keys and values should be
        strings or types that can be cast to a string via str(). Returns a boolean
    ...
    def pexpire(
        ...
        """Set an expire flag on key ``name`` for ``time`` milliseconds with given
        ``option``. ``time`` can be represented by an integer or a Python timedelta
        object.
    ...
    def pexpireat(
        ...
        """Set an expire flag on key ``name`` with given ``option``. representing unix
        time in. can be represented. as an integer.
    ...
    def psetex(self, name: KeyT, time_ms: ExpiryT, value: EncodableT):
        ...
        milliseconds. ``time_ms`` can be represented by an integer or a Python timedelta
        object.
    ...
    def restore(
        ...
        """Create a key using the provided serialized value, previously obtained using
        DUMP.
    ...
    def setbit(self, name: KeyT, offset: int, value: int) -> ResponseT:
        """Flag the ``offset`` in ``name`` as ``value``. integer. indicating the
        previous value. of ``offset``. Returns an.
    ...
    def setrange(self, name: KeyT, offset: int, value: EncodableT) -> ResponseT:
        ...
        ``value``. If ``offset`` plus the length of ``value`` exceeds the length of the
        original value, the new value will be larger than before. If ``offset`` exceeds
        the length of the original value, null bytes will be used to pad between the end
        of the previous value and the start of what's being injected.
    ...
    def stralgo(
        ...
        **kwargs,
        ...
        """Implements complex algorithms that operate on strings. is the LCS algorithm
        (longest common substring). However Right now the only new algorithm implemented
        algorithms could be implemented in the future.
    ...
    def touch(self, *args: KeyT) -> ResponseT:
        """Alters the last access time of a key(s) ``*args``. not exist. A. key is
        ignored. if it does.
...
class ListCommands(CommandsProtocol):
    ...
    def brpoplpush(
        ...
        """Pop a value off the tail of ``src``, push it on the head of ``dst`` and then
        return it.
    ...
    def blmpop(
        ...
        *args: List[str],
        ...
        """Pop ``count`` values (default 1) from first non-empty in the list of provided
        key names.

        When all lists are empty this command blocks the connection until another client
        pushes to it or until the timeout, timeout of 0 blocks indefinitely
    ...
    def lmpop(
        ...
        *args: List[str],
        ...
        """Pop ``count`` values (default 1) first non-empty list key from the list of
        args provided key names.
    ...
    def ltrim(self, name: str, start: int, end: int) -> Union[Awaitable[str], str]:
        """Trim the list ``name``, removing all values not within the slice between
        ``start`` and ``end``
    ...
    def rpoplpush(self, src: str, dst: str) -> Union[Awaitable[str], str]:
        """RPOP a value off of the ``src`` list and atomically LPUSH it on to the
        ``dst`` list.  Returns the value.
    ...
    def sort_ro(
        ...
        """Returns the elements contained in the list, set or sorted set at key. variant
        of the SORT command) (read- only.
...
class ScanCommands(CommandsProtocol):
    ...
    def scan(
        ...
        **kwargs,
        ...
        """Incrementally return lists of key names. position. Also. return a cursor.
        indicating the scan.
    ...
    def scan_iter(
        ...
        **kwargs,
        ...
        """Make an iterator using the SCAN command so that the client doesn't need to
    ...
    def sscan(
        ...
        """Incrementally return lists of elements in a set. cursor. indicating the scan.
        position. Also return a.
    ...
    def sscan_iter(
        ...
        """Make an iterator using the SSCAN command so that the client doesn't need to
        remember the cursor position.
    ...
    def hscan(
        ...
        """Incrementally return key/value slices in a hash. cursor. indicating the scan.
        position. Also return a.
    ...
    def hscan_iter(
        ...
        """Make an iterator using the HSCAN command so that the client doesn't need to
        remember the cursor position.
    ...
    def zscan(
        ...
        """Incrementally return lists of elements in a sorted set. return a. cursor
        indicating the. scan position. Also.
    ...
    def zscan_iter(
        ...
        """Make an iterator using the ZSCAN command so that the client doesn't need to
        remember the cursor position.
...
class SetCommands(CommandsProtocol):
    ...
    def sdiffstore(
        ...
        """Store the difference of sets specified by ``keys`` into a new set named
    ...
    def sinterstore(
        ...
        """Store the intersection of sets specified by ``keys`` into a new set named
        ``dest``.  Returns the number of keys in the new set.
...
class SortedSetCommands(CommandsProtocol):
    ...
    def zadd(
        ...
        """Set any number of element-name, score pairs to the key ``name``. a dict of
        element-names keys to score values. Pairs. are. specified as.
    ...
    def zcount(self, name: KeyT, min: ZScoreBoundT, max: ZScoreBoundT) -> ResponseT:
        """Returns the number of elements in the sorted set at key ``name`` with a score
        between ``min`` and ``max``.
    ...
    def zdiff(self, keys: KeysT, withscores: bool = False) -> ResponseT:
        """Returns the difference between the first and all successive input sorted sets
        provided in ``keys``.
    ...
    def zdiffstore(self, dest: KeyT, keys: KeysT) -> ResponseT:
        ...
        sets provided in ``keys`` and stores the result in ``dest``.
    ...
    def zinter(
        ...
        ``aggregate`` option, it is possible to specify how the results of the union are
        With the aggregated. This option defaults to SUM, where the score of an element
        is summed across the inputs where it exists. When this option is set to either
        MIN or MAX, the resulting set will contain the minimum or maximum score of an
    ...
    def zinterstore(
        ...
        """Intersect multiple sorted sets specified by ``keys`` into a new sorted set,
        ``dest``. Scores in the destination will be aggregated based on the
        ``aggregate``. This option defaults to SUM, where the score of an element is
        summed across the inputs where it exists. When this option is set to either MIN
        or MAX, the resulting set will contain the minimum or maximum score of an
        element across the inputs where it exists.
    ...
    def zintercard(
        ...
        ``keys``. When LIMIT provided (defaults to 0 and means unlimited), if the
        intersection cardinality reaches limit partway through the computation, the
    ...
    def zpopmax(self, name: KeyT, count: Optional[int] = None) -> ResponseT:
        """Remove and return up to ``count`` members with the highest scores from the
        sorted set ``name``.
    ...
    def zpopmin(self, name: KeyT, count: Optional[int] = None) -> ResponseT:
        """Remove and return up to ``count`` members with the lowest scores from the
        sorted set ``name``.
    ...
    def bzpopmax(self, keys: KeysT, timeout: TimeoutSecT = 0) -> ResponseT:
        """ZPOPMAX a value off of the first non-empty sorted set named in the ``keys``
        list.
    ...
    def bzpopmin(self, keys: KeysT, timeout: TimeoutSecT = 0) -> ResponseT:
        """ZPOPMIN a value off of the first non-empty sorted set named in the ``keys``
        list.
    ...
    def zmpop(
        ...
        """Pop ``count`` values (default 1) off of the first non-empty sorted set named
        in the ``keys`` list.
    ...
    def zrange(
        ...
        """Return a range of values from sorted set ``name`` between ``start`` and
        ``end`` sorted in ascending order.
    ...
    def zrevrange(
        ...
        """Return a range of values from sorted set ``name`` between ``start`` and
        ``end`` sorted in descending order.
    ...
    def zrangestore(
        ...
        """Stores in ``dest`` the result of a range of values from sorted set ``name``
        between ``start`` and ``end`` sorted in ascending order.
    ...
    def zrangebylex(
        ...
        """Return the lexicographical range of values from sorted set ``name`` between
        ``min`` and ``max``.
    ...
    def zrevrangebylex(
        ...
        """Return the reversed lexicographical range of values from sorted set ``name``
        between ``max`` and ``min``.
    ...
    def zrangebyscore(
        ...
        """Return a range of values from the sorted set ``name`` with scores between
        ``min`` and ``max``.
    ...
    def zrank(
        ...
        ``name``. The optional WITHSCORE argument supplements the command's reply with
        the score of the element returned.
    ...
    def zremrangebylex(self, name: KeyT, min: EncodableT, max: EncodableT) -> ResponseT:
        """Remove all elements in the sorted set ``name`` between the lexicographical
    ...
    def zremrangebyrank(self, name: KeyT, min: int, max: int) -> ResponseT:
        """Remove all elements in the sorted set ``name`` with ranks between ``min`` and
        ``max``. Values are 0-based, ordered from smallest score to largest. Values can
        be negative indicating the highest scores. Returns the number of elements
        removed.
    ...
    def zremrangebyscore(
        ...
        """Remove all elements in the sorted set ``name`` with scores between ``min``
        and ``max``. Returns the number of elements removed.
    ...
    def zrevrank(
        ...
        """Returns a 0-based value indicating the descending rank of ``value`` in sorted
        set ``name``. The optional ``withscore`` argument supplements the command's
    ...
    def zunion(
        ...
        """Return the union of multiple sorted sets specified by ``keys``. as dictionary
        of keys and their weights. Scores will be aggregated ``keys`` can be provided
        based on the ``aggregate``, or SUM if none is provided.
    ...
    def zunionstore(
        ...
        """Union multiple sorted sets specified by ``keys`` into a new sorted set,
        ``dest``. Scores in the destination will be aggregated based on the
        ``aggregate``, or SUM if none is provided.
    ...
    def zmscore(self, key: KeyT, members: List[str]) -> ResponseT:
        ...
        stored at key. ``members`` should be a list of the member name. Return type is a
        list of score. If the member does not exist, a None will be returned in
...
class HyperlogCommands(CommandsProtocol):
    ...
    def pfcount(self, *sources: KeyT) -> ResponseT:
        """Return the approximated cardinality of the set observed by the HyperLogLog at
        key(s).
...
class HashCommands(CommandsProtocol):
    ...
    def hgetdel(
        ...
        """Return the value of ``key`` within the hash ``name`` and delete the field in
        the hash. This command is similar to HGET, except for the fact that it also
        deletes the key on success from the hash with the provided ```name```.

        Available since Redis 8.0 For more information see
    ...
    def hgetex(
        ...
        *keys: str,
        ...
        """Return the values of ``key`` and ``keys`` within the hash ``name`` and
        optionally set their expiration.
    ...
    def hset(
        ...
        """Set ``key`` to ``value`` within hash ``name``, ``mapping`` accepts a dict of
        key/value pairs that will be added to hash ``name``. ``items`` accepts a list of
        key/value pairs that will be added to hash ``name``. Returns the number of
    ...
    def hmset(self, name: str, mapping: dict) -> Union[Awaitable[str], str]:
        """Set key to value within hash ``name`` for each corresponding key and value
        from the ``mapping`` dict.
    ...
    def hstrlen(self, name: str, key: str) -> Union[Awaitable[int], int]:
        """Return the number of bytes stored in the value of ``key`` within hash
        ``name``
    ...
    def hexpire(
        ...
        *fields: str,
        ...
        """Sets or updates the expiration time for fields within a hash key, using
    ...
    def hpexpiretime(self, key: KeyT, *fields: str) -> ResponseT:
        """Returns the expiration times of hash fields as Unix timestamps in
        milliseconds.
    ...
    def httl(self, key: KeyT, *fields: str) -> ResponseT:
        """Returns the TTL (Time To Live) in seconds for each specified field within a
        hash key.
    ...
    def hpttl(self, key: KeyT, *fields: str) -> ResponseT:
        """Returns the TTL (Time To Live) in milliseconds for each specified field
        within a hash key.
...
class PubSubCommands(CommandsProtocol):
    ...
    def publish(self, channel: ChannelT, message: EncodableT, **kwargs) -> ResponseT:
        """Publish ``message`` on ``channel``. number of subscribers the. message was
        delivered to. Returns the.
    ...
    def spublish(self, shard_channel: ChannelT, message: EncodableT) -> ResponseT:
        """Posts a message to the given shard channel. Returns the number of clients.
    ...
    def pubsub_numsub(self, *args: ChannelT, **kwargs) -> ResponseT:
        """Return a list of (channel, number of subscribers) tuples for each channel
        given in ``*args``
    ...
    def pubsub_shardnumsub(self, *args: ChannelT, **kwargs) -> ResponseT:
        """Return a list of (shard_channel, number of subscribers) tuples for each
        channel given in ``*args``
...
class ScriptCommands(CommandsProtocol):
    ...
    def eval(
        ...
        """Execute the Lua ``script``, specifying the ``numkeys`` the script will touch
        and the key names and argument values in ``keys_and_args``. Returns the result
        of the script.
    ...
    def evalsha(
        ...
        """Use the ``sha`` to execute a Lua script already registered via EVAL or SCRIPT
        LOAD. Specify the ``numkeys`` the script will touch and the key names and
        argument values in ``keys_and_args``. Returns the result of the script.
    ...
    def script_exists(self, *args: str) -> ResponseT:
        """Check if a script exists in the script cache by specifying the SHAs of each
        script as ``args``. Returns a list of boolean values indicating if if each
        already script exists in the cache_data.
    ...
    def script_load(self, script: ScriptTextT) -> ResponseT:
        ...
        Returns the SHA.         For more information see
    ...
    def register_script(self: "redis.client.Redis", script: ScriptTextT) -> Script:
        ...
        Returns a Script object that is callable and hides the complexity of deal with
        scripts, keys, and shas. This is the preferred way to work with Lua scripts.
        """
...
class GeoCommands(CommandsProtocol):
    ...
    def geoadd(
        ...
        ``name`` argument. The Geospatial items are given as ordered members of the
        ``values`` argument, each item or place is formed by the triad longitude,
        latitude and name.
    ...
    def geohash(self, name: KeyT, *values: FieldT) -> ResponseT:
        """Return the geo hash string for each item of ``values`` members of the
        specified key identified by the ``name`` argument.
    ...
    def geopos(self, name: KeyT, *values: FieldT) -> ResponseT:
        """Return the positions of each item of ``values`` as members of the specified
        key identified by the ``name`` argument. Each position is represented by the
        pairs lon and lat.
    ...
    def georadius(
        ...
        """Return the members of the specified key identified by the ``name`` argument
        which are within the borders of the area specified with the ``latitude`` and
        ``longitude`` location and the maximum distance from the center specified by the
        ``radius`` value.
    ...
    def georadiusbymember(
        ...
        instead of taking, as the center of the area to query, a longitude and latitude
    ...
    def geosearch(
        ...
        """Return the members of specified key identified by the ``name`` argument,
        which are within the borders of the area specified by a given shape. This
        command extends the GEORADIUS command, so in addition to searching within
    ...
    def geosearchstore(
        ...
        """This command is like GEOSEARCH, but stores the result in ``dest``. it stores
        the results in the destination sorted set with their By geospatial default,
        information. if ``store_dist`` set to True, the command will stores the items in
        a sorted set populated with their distance from the center of the circle or box,
...
class ModuleCommands(CommandsProtocol):
    ...
    def module_load(self, path, *args) -> ResponseT:
        """Loads the module from ``path``. is not found at ``path``. Passes all
        ``*args`` to the module, during. loading. Raises ``ModuleError`` if a module.
    ...
    def module_loadex(
        ...
        """Loads a module from a dynamic library at runtime with configuration
        directives.
    ...
    def module_unload(self, name) -> ResponseT:
        """Unloads the module ``name``. ``name`` is not in. loaded modules. Raises
        ``ModuleError`` if.
    ...
    def module_list(self) -> ResponseT:
        """Returns a list of dictionaries containing the name and version of all loaded
        modules.
...
class FunctionCommands:
    ...
    def function_load(
        ...
        """Load a library to Redis. :param code: the source code (must start with
        Shebang statement that provides a metadata about the library) :param replace:
        changes the behavior to overwrite the existing library with the new contents.
    ...
    def fcall_ro(
        ...
        """This is a read-only variant of the FCALL command that cannot execute commands
        that modify data.
    ...
    def function_restore(
        ...
        """Restore libraries from the serialized ``payload``. a policy for handling
        existing libraries. You can use the optional. policy argument to provide.
...
class CoreCommands(
    ...
    """A class containing all of the implemented redis commands.
