...
try:
    ...
    def match_hostname(cert, hostname):
        ...
        matches the *hostname*.  RFC 2818 and RFC 6125 rules are followed, but IP
...
try:
    ...
    def which(cmd, mode=os.F_OK | os.X_OK, path=None):
        """Given a command, mode, and a PATH string, return the path which conforms to
        the given mode on the PATH, or None if there is no such file.
...
try:
    ...
    def detect_encoding(readline):
        ...
        used to decode a Python source file.  It requires one argument, readline, in the
        same way as the tokenize() generator.

        It will call readline a maximum of twice, and return the encoding used (as a
        string) and a list of any lines (left as bytes) it has read in.

        It detects the encoding from the presence of a utf-8 bom or an encoding cookie
        as specified in pep-0263.  If both a bom and a cookie are present, but disagree,
        a SyntaxError will be raised.  If the encoding cookie is an invalid charset,
        raise a SyntaxError.  Note that if a utf-8 bom is found, 'utf-8-sig' is
        returned.
...
try:
    ...
    class OrderedDict(dict):
        ...
        def __init__(self, *args, **kwds):
            ...
            Signature is the same as for regular dictionaries, but keyword arguments are
        ...
        def fromkeys(cls, iterable, value=None):
            """OD.fromkeys(S[, v]) -> New ordered dictionary with keys from S and values
            equal to v (which defaults to None)."""
            d = cls()
            for key in iterable:
                d[key] = value
            return d
        ...
        def __eq__(self, other):
            ...
            Comparison to another OD is order-sensitive while comparison to a regular
            mapping is order-insensitive.
            """
