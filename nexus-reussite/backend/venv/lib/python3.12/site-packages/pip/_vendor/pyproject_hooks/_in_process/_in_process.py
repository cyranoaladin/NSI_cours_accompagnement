...
class _BackendPathFinder:
    ...
    MetaPathFinders, the only way to ensure the backend is loaded from the right place
    is to prepend our own.
    """
...
def _supported_features():
    ...
    Returns a list of strings. The only possible value is 'build_editable'.
    """
...
def prepare_metadata_for_build_wheel(
    ...
    Implements a fallback by building a wheel if the hook isn't defined, unless
    _allow_fallback is False in which case HookMissing is raised.
    """
    backend = _build_backend()
    try:
...
def prepare_metadata_for_build_editable(
    ...
    Implements a fallback by building an editable wheel if the hook isn't defined,
...
def _get_wheel_metadata_from_wheel(whl_basename, metadata_directory, config_settings):
    ...
    Fallback for when the build backend does not define the 'get_wheel_metadata' hook.
    """
...
def build_editable(wheel_directory, config_settings, metadata_directory=None):
    ...
    If a wheel was already built in the prepare_metadata_for_build_editable fallback,
    this will copy it rather than rebuilding the wheel.
    """
    backend = _build_backend()
    try:
