...
class Limit:
    ...
    def scope_for(self, endpoint: str, method: str | None) -> str:
        """Derive final bucket (scope) for this limit given the endpoint and request
        method.

        If the limit is shared between multiple routes, the scope does not include the
        endpoint.
        """
...
@dataclasses.dataclass(eq=True, unsafe_hash=True)
class LimitGroup:
    """Represents a group of related limits either from a string or a callable that
    returns one."""
