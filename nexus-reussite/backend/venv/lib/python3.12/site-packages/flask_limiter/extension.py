...
class Limiter:
    ...
    def __init__(
        ...
        *,
        ...
        on_breach: None | (
            Callable[[RequestLimit], flask.wrappers.Response | None]
        ) = None,
    ...
    def context(self) -> LimiterContext:
        """The context is meant to exist for the lifetime of a request/response cycle
        per instance of the extension so as to keep track of any state used at different
        steps in the lifecycle (for example to pass information from the before request
        hook to the after_request hook)
    ...
    def request_filter(self, fn: Callable[[], bool]) -> Callable[[], bool]:
        """Decorator to mark a function as a filter to be executed to check if the
        request is exempt from rate limiting.

        :param fn: The function will be called before evaluating any rate limits to
            decide whether to perform rate limit or skip it.
        """
    ...
    def check(self) -> None:
        """Explicitly check the limits for the current request.
        :paramref:`~flask_limiter.Limiter.auto_check` This is only if the extension was
        initialized with relevant set to ``False``
    ...
    def current_limits(self) -> list[RequestLimit]:
        """Get a list of all rate limits that were applicable and evaluated within the
    ...
    def identify_request(self) -> str:
        ...
        :attr:`flask.Request.endpoint` associated by the view function that is handling
        the request).
...
class LimitDecorator:
    """Wrapper used by :meth:`~flask_limiter.Limiter.limit` and
    :meth:`~flask_limiter.Limiter.shared_limit` when wrapping view functions or
    blueprints."""
