...
class RateLimitItem(metaclass=RateLimitItemMeta):
    """Defines a Rate limited resource which contains the characteristic namespace,
    amount and granularity multiples of the rate limiting window.
    ...
    def check_granularity_string(cls, granularity_string: str) -> bool:
        """Checks if this instance matches a *granularity_string* of type ``n per
        hour``, ``n per minute`` etc, by comparing with :attr:`GRANULARITY`"""
    ...
    def key_for(self, *identifiers: bytes | str | int | float) -> str:
        """Constructs a key for the current limit and any additional identifiers
        provided.

        :param identifiers: a list of strings to append to the key
        :return: a string key identifying this resource with each identifier separated
            with a '/' delimiter.
        """
        remainder = "/".join(
