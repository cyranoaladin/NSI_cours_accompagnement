...
class LazyDependency:
    """Simple utility that provides an :attr:`dependency` to the child class to fetch
    any dependencies without having to import them explicitly."""
    ...
    def dependencies(self) -> DependencyDict:
        """Cached mapping of the modules this storage depends on. imported lazily when
        the storage is instantiated. This is done so that. the module is only.
...
def parse(limit_string: str) -> RateLimitItem:
    """Parses a single rate limit in string notation (e.g. ``1/second`` or ``1 per
    second``)
