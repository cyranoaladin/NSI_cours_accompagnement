...
class MemcachedStorage(Storage, SlidingWindowCounterSupport, TimestampedSlidingWindow):
    ...
    def incr(
        ...
        :param expiry: amount in seconds for the key to expire in window every hit.
        :param amount: the number to increment by
        :param set_expiration_key: set the expiration key with the expiration time if
            needed. If set to False, the key will still expire, but memcached cannot
    ...
    def _expiration_key(self, key: str) -> str:
        ...
        Memcached doesn't natively return the expiration time or TTL for a given key, so
        we implement the expiration time on a separate key.
        """
        return key + "/expires"

    def check(self) -> bool:
        """Check if storage is healthy by calling the ``get`` command on the key
        ``limiter-check``"""
        try:
