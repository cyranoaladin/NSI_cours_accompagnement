...
@versionchanged(
    version="3.14.0",
    reason=\
           """Dropped support for the :pypi:`redis-py-cluster` library which has been
...
@versionchanged(
    version="2.5.0",
    reason=\
           """Cluster support was provided by the :pypi:`redis-py-cluster` library which
           has been absorbed into the official :pypi:`redis` client.
...
class RedisClusterStorage(RedisStorage):
    ...
    def reset(self) -> int | None:
        """Redis Clusters are sharded and deleting across shards can't be done
        atomically. Because of this, this reset loops over all keys that are prefixed
        with :paramref:`RedisClusterStorage.prefix` and calls delete on them one at a
        time.
