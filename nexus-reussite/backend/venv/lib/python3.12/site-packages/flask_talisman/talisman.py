...
class Talisman(object):
    ...
    def init_app(
        ...
        self.strict_transport_security_preload = strict_transport_security_preload
        ...
        if (
            ...
            and self.content_security_policy_report_uri is None
        ):
            raise ValueError(
        ...
        self.content_security_policy_nonce_in = content_security_policy_nonce_in or []
    ...
    def _get_local_options(self):
        view_function = flask.current_app.view_functions.get(flask.request.endpoint)
        ...
        view_options.setdefault("content_security_policy", self.content_security_policy)
        view_options.setdefault(
            "content_security_policy_nonce_in", self.content_security_policy_nonce_in
        )
        view_options.setdefault("permissions_policy", self.permissions_policy)
        view_options.setdefault("document_policy", self.document_policy)
        view_options.setdefault("feature_policy", self.feature_policy)
    ...
    def _make_nonce(self):
        local_options = self._get_local_options()
        if (
            local_options["content_security_policy"]
            and local_options["content_security_policy_nonce_in"]
            and not getattr(flask.request, "csp_nonce", None)
        ):
    ...
    def _parse_policy(self, policy):
        ...
        for section, content in policy.items():
            ...
            if (
                hasattr(flask.request, "csp_nonce")
                and section in local_options["content_security_policy_nonce_in"]
            ):
