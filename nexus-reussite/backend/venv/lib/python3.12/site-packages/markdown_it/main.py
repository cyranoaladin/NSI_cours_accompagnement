...
class MarkdownIt:
    ...
    def set(self, options: OptionsType) -> None:
        """Set parser options (in the same format as in constructor). you can change
        options after constructor call. Probably, you will. never need it, but.

        __Note:__ To achieve the best possible performance, don't modify a `markdown-it`
        instance options on the fly. If you need multiple configurations it's best to
        create multiple instances and initialize each with separate config.
        """
    ...
    def renderInline(self, src: str, env: EnvType | None = None) -> Any:
        ...
        :param src: source string
        :param env: environment sandbox Similar to [[MarkdownIt.render]] but for single
            paragraph content. Result will NOT be wrapped into `<p>` tags.
        """
        env = {} if env is None else env
