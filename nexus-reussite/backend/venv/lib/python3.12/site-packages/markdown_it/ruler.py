...
class Ruler(Generic[RuleFuncTv]):
    ...
    def getRules(self, chainName: str = "") -> list[RuleFuncTv]:
        """Return array of active functions (rules) for given chain name. configuration,
        compiles caches if not exists and returns result. It analyzes. rules.

        Default chain name is `''` (empty string). It can't be skipped. That's done
        intentionally, to keep signature monomorphic for high speed.
        """
