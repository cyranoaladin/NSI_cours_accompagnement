...
class SyntaxTreeNode:
    ...
    def _set_children_from_tokens(self, tokens: Sequence[Token]) -> None:
        """Convert the token stream to a tree structure and set the resulting nodes as
        children of `self`."""
    ...
    def walk(
        ...
        The order mimics the order of the underlying linear token stream (i.e. depth
        first).
        """
        if include_self:
            yield self
    ...
    def _attribute_token(self) -> Token:
        """Return the `Token` that is used as the data source for the properties defined
        below."""
        if self.token:
            return self.token
    ...
    def content(self) -> str:
        """In a case of self-closing tag (code, html, fence, etc.), it has contents of
        this tag."""
...
def _removesuffix(string: str, suffix: str) -> str:
    ...
    Replace this with str.removesuffix() from stdlib when minimum Python version is 3.9.
    """
