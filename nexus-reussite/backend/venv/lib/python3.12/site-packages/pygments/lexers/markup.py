...
class GroffLexer(RegexLexer):
    """Lexer for the (g)roff typesetting language, supporting groff extensions.

    Mainly useful for highlighting manpage sources.
    """
...
class MarkdownLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (
                r"""(?x) ^(?P<initial>\s*```) (?P<lang>[\w\-]+) (?P<afterlang>
                (?P<whitespace>[^\S\n]+) (?P<extra>.*))?
...
class WikitextLexer(RegexLexer):
    ...
    installations, so we only highlight common syntaxes (built-in or from popular
    extensions), and also assume templates produce no unbalanced syntaxes.
    """
    ...
    tokens = {
        "root": [
            # Redirects
            (
                r"""(?xi) (\A\s*?)(\#REDIRECT:?) # may contain a colon (\s+)(\[\[)
                (?=[^\]\n]* \]\]$)"""
                                     ,
            ...
            (
                r"""(?x) \bISBN {nbsp_char} (?: 97[89] {nbsp_dash}?

                )? (?: [0-9] {nbsp_dash}? ){{9}} # escape format() [0-9Xx]\b
                """
                   .format(
