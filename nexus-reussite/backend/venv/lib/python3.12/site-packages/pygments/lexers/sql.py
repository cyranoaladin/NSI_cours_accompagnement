...
def language_callback(lexer, match):
    ...
    The lexer is chosen looking for a nearby LANGUAGE or assumed as plpgsql if inside a
    DO statement and no LANGUAGE has been found.
    """
    lx = None
...
class PostgresBase:
    ...
    This is implemented as a mixin to avoid the Lexer metaclass kicking in. this way the
    different lexer don't have a common Lexer ancestor. If they had, _tokens could be
    created on this ancestor and not updated for the other classes, resulting e.g. in
    PL/pgSQL parsed as SQL. This shortcoming seem to suggest that regexp lexers are not
    really subclassable.
    """
...
class PsqlRegexLexer(PostgresBase, RegexLexer):
    ...
    This is not a complete psql lexer yet as it lacks prompt support and output
    rendering.
    """
...
class SqlLexer(RegexLexer):
    ...
    Currently, this lexer does not recognize any special syntax except ANSI SQL.
    """
...
class MySqlLexer(RegexLexer):
    ...
    This lexer does not attempt to maintain strict compatibility with MariaDB syntax or
    keywords. Although MySQL and MariaDB's common code history suggests there may be
    significant overlap between the two, compatibility between the two is not a target
    for this lexer.
    """
