...
def _next_term(text: str, openmetrics: bool) -> Tuple[str, str]:
    ...
    Returns the stripped term and the stripped remainder of the string, including the
    comma.
...
def _next_unquoted_char(text: str, chs: str, startidx: int = 0) -> int:
    ...
    It is always assumed that the first character being checked is not already inside
    quotes.
    """
    i = startidx
    in_quotes = False
...
def _split_quoted(text, separator, maxsplit=0):
    """Splits on split_ch similarly to strings.split, skipping separators if they are
    inside quotes."""
    tokens = [""]
    x = 0
    while x < len(text):
