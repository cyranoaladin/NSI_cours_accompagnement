...
class _PrometheusRedirectHandler(HTTPRedirectHandler):
    ...
    def redirect_request(self, req, fp, code, msg, headers, newurl):
        ...
        If the redirect is disallowed, this raises the corresponding HTTP error. If the
        redirect can't be determined, return None to allow other handlers to try. If the
        redirect is allowed, return the new request.

        This method specialized for the case when (a) the user knows that the redirect
        will not cause unacceptable side effects for any request method, and (b) the
        user knows that any request data should be passed through to the redirect. If
        either condition is not met, this should not be used.
        """
...
def write_to_textfile(path: str, registry: CollectorRegistry) -> None:
    ...
    This is intended for use with the Node exporter textfile collector. The path must
    end in .prom for the textfile collector to process it.
    """
...
def passthrough_redirect_handler(
    ...
    preserving the method upon redirect, and passing through all headers and data from
    the original request. Only use this handler if you control or trust the source of
    redirect responses you encounter when making requests via the Prometheus client.
    This handler will simply repeat the identical request, including same method and
    data, to the new redirect URL.
    """
...
def basic_auth_handler(
    ...
    Sets auth headers using supplied 'username' and 'password', if set. Used by the
    push_to_gateway functions. Can be re-used by other handlers.
    """
...
def tls_auth_handler(
    ...
    The default protocol (ssl.PROTOCOL_TLS_CLIENT) will also enable ssl.CERT_REQUIRED
    and SSLContext.check_hostname by default. This can be disabled by setting
