...
def is_storage_device(name):
    """Return True if the given name refers to a root device (e.g. "sda", "nvme0n1") as
    opposed to a logical partition (e.g.  "sda1", "nvme0n1p1").

    If name is a virtual device (e.g. "loop1", "ram") return True.
    """
...
def per_cpu_times():
    """Return a list of namedtuple representing the CPU times for every CPU available on
    the system."""
    procfs_path = get_procfs_path()
    set_scputimes_ntuple(procfs_path)
    cpus = []
...
if os.path.exists("/sys/devices/system/cpu/cpufreq/policy0") or os.path.exists(
    ...
    def cpu_freq():
        ...
        Contrarily to other OSes, Linux updates these values in real-time.
        """
...
class NetConnections:
    """A wrapper on top of /proc/net/* files, retrieving per-process and system-wide
    open connections (TCP, UDP, UNIX) similarly to "netstat -an".
    ...
    def decode_address(addr, family):
        """Accept an "ip:port" address as displayed in /proc/net/* and convert it into a
...
def net_io_counters():
    """Return network I/O statistics for every network interface installed on the system
    as a dict of raw tuples."""
...
class RootFsDeviceFinder:
    """disk_partitions() may return partitions with device == "/dev/root" or "rootfs".

    This container class uses different strategies to try to obtain the real device
    path. Resources:
...
def sensors_temperatures():
    """Return hardware (CPU and others) temperatures as a dict including hardware name,
    label, current, max and critical temperatures.
...
def sensors_fans():
    ...
    hardware label and current speed.
...
def sensors_battery():
    ...
    def multi_bcat(*paths):
        """Attempt to read the content of multiple files which may not exist.
...
def pid_exists(pid):
    ...
    Linux TIDs are not supported (always return False).
    """
...
def wrap_exceptions(fun):
    """Decorator which translates bare OSError and OSError exceptions into NoSuchProcess
    and AccessDenied."""
...
class Process:
    ...
    def _parse_stat_file(self):
        """Parse /proc/{pid}/stat file and return a dict with various process info.

        Using "man proc" as a reference: where "man proc" refers to position N always
        subtract 3 (e.g ppid position 4 in 'man proc' == position 1 in here). The return
        value is cached in case oneshot() ctx manager is in use.
        """
        data = bcat(f"{self._procfs_path}/{self.pid}/stat")
    ...
    def _read_status_file(self):
        ...
        The return value is cached in case oneshot() ctx manager is in use.
        """
