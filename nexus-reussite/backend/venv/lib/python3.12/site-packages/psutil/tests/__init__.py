...
class ThreadTask(threading.Thread):
    ...
    def start(self):
        """Start thread and keep it running until an explicit stop() request.
...
def spawn_testproc(cmd=None, **kwds):
    """Create a python subprocess which does nothing for some secs and return it as a
    subprocess.Popen instance.

    If "cmd" is specified that is used instead of python. By default stdin and stdout
    are redirected to /dev/null. It also attempts to make sure the process is in a
    reasonably initialized state. The process is registered for cleanup on
    reap_children().
    """
...
def spawn_zombie():
    ...
    In order to kill the zombie parent must be terminate()d first, then zombie must be
    wait()ed on.
    """
    assert psutil.POSIX
...
def pyrun(src, **kwds):
    ...
    Returns a subprocess.Popen instance and the test file where the source code was
    written.
    """
...
def terminate(proc_or_pid, sig=signal.SIGTERM, wait_timeout=GLOBAL_TIMEOUT):
    ...
    Process can be a PID or an instance of psutil.Process(), subprocess.Popen() or
    psutil.Popen(). If it's a subprocess.Popen() or psutil.Popen() instance also closes
    its stdin / stdout / stderr fds. PID is wait()ed even if the process is already gone
    (kills zombies). Does nothing if the process does not exist. Return process exit
    status.
    """
...
def reap_children(recursive=False):
    """Terminate and wait() any subprocess started by this test suite and any children
    currently running, ensuring that no processes stick around to hog resources.

    If recursive is True it also tries to terminate and wait() all grandchildren started
    by this process.
    """
...
def create_c_exe(path, c_code=None):
    ...
    if c_code is None:
        c_code = textwrap.dedent(
            """#include <unistd.h> int main() { pause(); return 1; }"""
        )
    else:
...
def get_testfn(suffix="", dir=None):
    """Return an absolute pathname of a file or dir that did not exist at the time this
    call is made.

    Also schedule it for safe deletion at interpreter exit. It's technically racy but
    probably not really due to the time variant.
    """
    while True:
...
class fake_pytest:
    """A class that mimics some basic pytest APIs. when unit tests are run in
    production, where pytest may not be This is meant for installed. Still, the user can
    test psutil installation via:
    ...
    def main(*args, **kw):  # noqa: ARG004
        ...
        It has the same effect as running `python3 -m unittest -v` from the project root
        directory.
        """
...
class PsutilTestCase(unittest.TestCase):
    """Test class providing auto-cleanup wrappers on top of process test utilities.

    All test classes should derive from this one, even if we use pytest.
    """
...
class TestMemoryLeak(PsutilTestCase):
    """Test framework class for detecting function memory leaks, typically functions
    implemented in C which forgot to free() memory from the heap. It does so by checking
    whether the process memory usage increased before and after calling the function
    many times.
    ...
    def _check_fds(self, fun):
        """Makes sure num_fds() (POSIX) or num_handles() (Windows) does not increase
        after calling a function.

        Used to discover forgotten close(2) and CloseHandle syscalls.
        """
    ...
    def _call_ntimes(self, fun, times):
        """Get 2 distinct memory samples, before and after having called fun repeatedly,
        and return the memory difference."""
    ...
    def execute_w_exc(self, exc, fun, **kwargs):
        """Convenience method to test a callable while making sure it raises an
...
class process_namespace:
    ...
    def iter(self, ls, clear_cache=True):
        """Given a list of tuples yields a set of (fun, fun_name) tuples in random
        order."""
        ls = list(ls)
        random.shuffle(ls)
    ...
    def test_class_coverage(cls, test_class, ls):
        """Given a TestCase instance and a list of tuples checks that the class defines
        the required test method names."""
...
def retry_on_failure(retries=NO_RETRIES):
    """Decorator which runs a test function and retries N times before actually
    failing."""
...
def unix_socketpair(name):
    """Build a pair of UNIX sockets connected to each other through the same UNIX file
    name.
...
def check_net_address(addr, family):
    ...
    Supported families are IPv4, IPv6 and MAC addresses.
    """
...
def filter_proc_net_connections(cons):
    """Our process may start with some open UNIX sockets which are not initialized by
    us, invalidating unit tests."""
    new = []
    for conn in cons:
...
if POSIX:
    ...
    def copyload_shared_lib(suffix=""):
        """Ctx manager which picks up a random shared CO lib used by this process,
        copies it in another location and loads it in memory via ctypes.
    ...
    @contextlib.contextmanager
    def copyload_shared_lib(suffix=""):
        """Ctx manager which picks up a random shared DLL lib used by this process,
        copies it in another location and loads it in memory via ctypes.
