...
class ConnectionTestCase(PsutilTestCase):
    ...
    def compare_procsys_connections(self, pid, proc_cons, kind="all"):
        """Given a process PID and its list of connections compare those against system-
        wide connections retrieved via psutil.net_connections."""
        try:
...
class TestUnconnectedSockets(ConnectionTestCase):
    ...
    def check_socket(self, sock):
        """Given a socket, makes sure it matches the one obtained via psutil.

        It assumes this process created one connection only (the one supposed to be
        checked).
        """
...
class TestConnectedSocket(ConnectionTestCase):
    """Test socket pairs which are actually connected to each other."""
